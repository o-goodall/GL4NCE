...
export default function BitcoinTicker() {
  ...
  useEffect(() => {
    ...
    fetchKlines(timeframe, ctrl.signal)
      ...
      .catch((err: unknown) => {
        ...
        if (import.meta.env.DEV) console.error("[BitcoinTicker] fetch error:", err);
        setLoading(false);
      });
    return () => ctrl.abort();
  }, [timeframe]);
  ...
  useEffect(() => {
    ...
    ws.onerror = () => {
      if (import.meta.env.DEV) console.error("[BitcoinTicker] WebSocket error");
    };
  ...
  // ── Render ───────────────────────────────────────────────────────────────────
  return (
    <div className="relative rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] sm:p-6">
      {/* Tile badge */}
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 mb-4 pr-8">
        <div>
      ...
      {/* Chart area */}
      <div className="relative -mx-1 h-[220px] sm:h-[300px]" aria-label="Bitcoin price chart">
        {loading ? (
